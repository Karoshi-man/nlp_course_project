...
![Status](https://img.shields.io/badge/Status-Lab%204%3A%20Rule--based%20IE-orange)

> **📍 Поточна гілка: `lab-04`**
> Ця гілка містить фінальний код для Лабораторної роботи №4. Головний фокус — детерміноване витягування сутностей (Information Extraction) за допомогою регулярних виразів та словників (Rule-based підхід). Реалізовано парсинг полів `SALARY`, `EXPERIENCE_YEARS` та `ENGLISH_LEVEL` із фокусом на метрику Precision (мінімізація хибних спрацювань). Звіти та Error Analysis знаходяться у `docs/audit_summary_lab4.md`.
...
- [x] **Lab 4:** Реалізовано Rule-based Information Extraction (Regex + Dictionaries) для витягування полів SALARY, EXPERIENCE_YEARS, ENGLISH_LEVEL. Досягнуто 100% Precision на Gold Subset.
...
3. **Запустіть пайплайн Information Extraction:**
   Відкрийте файл `notebooks/lab4_rule_based_ie.ipynb` у Jupyter або VS Code та виконайте всі клітинки ("Run All"). Ноутбук автоматично імпортує правила з `src/ie_rules.py`, прожене через них еталонні дані та виведе метрики якості (Precision) з аналізом помилок.
